...
def local_llama_config_option_schema(
    ...
    if subentry_type == ai_task.DOMAIN:
        ...
        result.update({
            ...
            vol.Optional(
                CONF_ENABLE_FOLLOW_UP_CONVERSATION,
                description={"suggested_value": options.get(CONF_ENABLE_FOLLOW_UP_CONVERSATION, DEFAULT_ENABLE_FOLLOW_UP_CONVERSATION)},
                default=options.get(CONF_ENABLE_FOLLOW_UP_CONVERSATION, DEFAULT_ENABLE_FOLLOW_UP_CONVERSATION),
            ): BooleanSelector(BooleanSelectorConfig()),
            vol.Optional(
                CONF_CONTINUE_CONVERSATION_MARKER,
                description={"suggested_value": options.get(CONF_CONTINUE_CONVERSATION_MARKER, DEFAULT_CONTINUE_CONVERSATION_MARKER)},
                default=options.get(CONF_CONTINUE_CONVERSATION_MARKER, DEFAULT_CONTINUE_CONVERSATION_MARKER),
        ...
        # Both shown/hidden based on the currently saved option one level up
        # - like other conditional fields in this schema, each appears the
        # next time the options are opened after saving its parent toggle
        # on, not live within the same form. No reason to ask how to deliver
        # follow-up examples for a feature that's off, or which file to load
        # examples from if they're not being sent as messages at all (the
        # system-prompt-prose path always uses whatever's loaded, with no
        # per-install override needed there).
        if options.get(CONF_ENABLE_FOLLOW_UP_CONVERSATION, DEFAULT_ENABLE_FOLLOW_UP_CONVERSATION):
            result[vol.Optional(
                CONF_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES,
                description={"suggested_value": options.get(CONF_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES, DEFAULT_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES)},
                default=options.get(CONF_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES, DEFAULT_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES),
            )] = BooleanSelector(BooleanSelectorConfig())

        if options.get(CONF_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES, DEFAULT_SEND_FOLLOW_UP_EXAMPLES_AS_MESSAGES):
            result[vol.Optional(
                CONF_FOLLOW_UP_EXAMPLES_FILE,
